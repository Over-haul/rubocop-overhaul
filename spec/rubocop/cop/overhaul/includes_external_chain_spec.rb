...
RSpec.describe RuboCop::Cop::Overhaul::IncludesExternalChain, :config do
  ...
  context "with safe chains" do
    ...
    it "allows Enumerable .select with a block after includes_external" do
      expect_no_offenses(<<~RUBY)
        ...
        Fleet.where(active: true).includes_external(:am_asset).select(&:active?)
      RUBY
    end
  ...
  context "with numbered/it-parameter blocks and parenthesized receivers" do
    let(:ruby_version) { 3.4 }

    it "allows Enumerable .select with a numbered-parameter block" do
      expect_no_offenses(<<~RUBY)
        Fleet.includes_external(:am_asset).select { _1.tractor? }
      RUBY
    end

    it "allows Enumerable .select with an it-parameter block" do
      expect_no_offenses(<<~RUBY)
        Fleet.includes_external(:am_asset).select { it.tractor? }
      RUBY
    end

    it "registers an offense across a numbered-parameter block in the chain" do
      expect_offense(<<~RUBY)
        Fleet.includes_external(:am_asset).tap { _1 }.where(id: 1)
                                                      ^^^^^ Do not chain `where` after `includes_external` — it returns a new relation that re-queries without the external preload. Move `where` before `includes_external`.
      RUBY
    end

    it "registers an offense when the receiver is wrapped in parentheses" do
      expect_offense(<<~RUBY)
        (Fleet.includes_external(:am_asset)).where(id: 1)
                                             ^^^^^ Do not chain `where` after `includes_external` — it returns a new relation that re-queries without the external preload. Move `where` before `includes_external`.
      RUBY
    end
  end
